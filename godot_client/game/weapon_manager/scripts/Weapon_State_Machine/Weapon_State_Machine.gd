...
#@export_enum('hitscan') var types = 'hitscan'
...
func _ready() -> void:
	...
	# CRITICAL: This file sucks and breaks on web with String == int issues.
	# CRITICAL: #TODO: REWRITE THIS ENTIRE FUCKING THING. I AM TIRED OF IT
...
func load_projectile(spread, source):
	...
	# NOTE: Do not edit this file any further. 
	
	# CRITICAL: Had to do it old school without a custom spawn function due to WebRTC 
	#print('DEBUG PROJECTILE: ', is_multiplayer_authority(), is_inside_tree(), multiplayer.get_peers())
	#projectile_system.spawner.spawn(projectile_data)
	spawn_projectile_web_rtc.rpc(projectile_data)

@rpc("any_peer", 'call_local')
func spawn_projectile_web_rtc(projectile_data):
	# CRITICAL: Game development is a mental illness.
	#if multiplayer.is_server():
	#Hub.world.add_new_ball()
	var new_projectile = Hub.projectile_system.handle_projectile_spawn(projectile_data)
	new_projectile.set_multiplayer_authority(multiplayer.get_unique_id())
	Hub.projectile_system.container.add_child(new_projectile, true)
...
func shoot():
	...
	if animation_player and not animation_player.is_playing():
		...
		#if current_weapon.projectile_type == PROJECTILE_TYPES.hitscan:
			#camera_hitscan()
		#else:

		load_projectile(spread, int(player.name))
